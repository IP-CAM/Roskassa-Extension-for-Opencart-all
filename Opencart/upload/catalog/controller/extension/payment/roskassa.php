...
class ControllerExtensionPaymentRoskassa extends Controller
{
    ...
    public function index()
    {
        ...
        $data['sid'] = $this->config->get('payment_roskassa_account');
        $roskassa_secret = $this->config->get('payment_roskassa_secret');
        ...
        $data['roskassa_test'] = $roskassa_test;
    ...
    public function success()
    {
        ...
        $this->model_checkout_order->addOrderHistory($_REQUEST['order_id'], $this->config->get('payment_roskassa_order_status_id'), 'Order confirmed');
    ...
    public function callback()
    {
        ...
        if ($MNT_TRANSACTION_ID && $MNT_SIGNATURE) {
            $mnt_dataintegrity_code = $this->config->get('payment_roskassa_secret');
            $sign_arr = array(
                'shop_id' => $MNT_ID,           
            ...
            if ($MNT_SIGNATURE == $check_signature) {
                ...
                if ($order_info) {
                    $this->model_checkout_order->addOrderHistory($order_id, $this->config->get('payment_roskassa_order_status_id'), 'Payment complete');
                    ...
                    foreach ($orderTotals as $orderTotal)
                        ...
                        self::monetaPayURLResponse($_REQUEST['shop_id'], $_REQUEST['order_id'],
